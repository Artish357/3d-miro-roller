...
class DieActionValueError extends Error {
  ...
  constructor(die, action = null) {
    super(
      `Die "${die}" must have more than 1 possible value to ${action || "do this action"}`
    );
...
const isSafeNumber = (val) => {
  ...
  return (
    castVal <= Number.MAX_SAFE_INTEGER && castVal >= Number.MIN_SAFE_INTEGER
  );
};
...
const nodeCrypto = {
  next() {
    if (index$1$1 >= COUNT$1) {
      data$1 = new Int32Array(
        new Int8Array(require("crypto").randomBytes(4 * COUNT$1)).buffer
      );
      index$1$1 = 0;
    }
...
class ComparePoint {
  ...
  static isValidOperator(operator) {
    return (
      typeof operator === "string" && /^(?:[<>!]?=|[<>]|<>)$/.test(operator)
    );
  }
  ...
  run(results, _context) {
    ...
    parsedResults.rolls = results.rolls
      .map((roll) => {
        ...
        for (
          let i = 0;
          i < this.maxIterations && this.isComparePoint(compareValue);
          i++
        ) {
...
class RollResult {
  ...
  constructor(value, modifiers = [], useInTotal = true) {
    if (isNumeric(value)) {
      ...
      const initialVal = isNumeric(value.initialValue)
        ? value.initialValue
        : value.value;
      ...
      this.useInTotal =
        typeof value.useInTotal === "boolean"
          ? value.useInTotal
          : useInTotal || false;
  ...
  set calculationValue(value) {
    ...
    this[calculationValueSymbol$1] = isValNumeric
      ? parseFloat(`${value}`)
      : null;
  }
  ...
  set modifiers(value) {
    ...
    throw new TypeError(
      `modifiers must be a Set or array of modifier names: ${value}`
    );
  }
   ...
   */
  get value() {
    return isNumeric(this[valueSymbol])
      ? this[valueSymbol]
      : this[initialValueSymbol];
  }
...
class RollResults {
  ...
  get value() {
    return this.rolls.reduce(
      (v, roll) => v + (roll.useInTotal ? roll.calculationValue : 0),
      0
    );
  }
...
class ReRollModifier extends ComparisonModifier {
  ...
  run(results, _context) {
    ...
    results.rolls.map((roll) => {
      ...
      for (
        let i = 0;
        i < this.maxIterations && this.isComparePoint(roll.value);
        i++
      ) {
        // re-roll the dice
        const rollResult = _context.rollOnce();
...
class StandardDice {
  ...
  get modifiers() {
    if (this[modifiersSymbol$2]) {
      ...
      return new Map(
        [...this[modifiersSymbol$2]].sort((a, b) => a[1].order - b[1].order)
      );
    }
  ...
  get notation() {
    ...
    if (this.modifiers && this.modifiers.size) {
      notation += [...this.modifiers.values()].reduce(
        (acc, modifier) => acc + modifier.notation,
        ""
      );
    }
...
class ResultGroup {
  ...
  get value() {
    ...
    const value = this.results.reduce(
      (v, result) => {
        let val = result;
...
class KeepModifier extends Modifier {
  ...
  run(results, _context) {
    ...
    if (results instanceof ResultGroup) {
      ...
      if (
        modifiedRolls.length === 1 &&
        modifiedRolls[0] instanceof ResultGroup
      ) {
        // single sub-roll - get all the dice rolled and their 2d indexes
        rollIndexes = modifiedRolls[0].results
          .map((result, index) => {
...
class RollGroup {
  ...
  set expressions(expressions) {
    ...
    expressions.forEach((expression) => {
      if (!expression || !Array.isArray(expression)) {
        throw new TypeError(
          `Expressions must be an array of arrays: ${expressions}`
        );
      }
  ...
  get modifiers() {
    if (this[modifiersSymbol]) {
      // ensure modifiers are ordered correctly
      return new Map(
        [...this[modifiersSymbol]].sort((a, b) => a[1].order - b[1].order)
      );
    }
...
function peg$parse(input, options) {
  ...
  var peg$f0 = function (expr, exprs, modifiers) {
    return new RollGroup(
      [expr, ...exprs.map((v) => v[3])],
      Object.assign(
        {},
        ...modifiers.map((item) => {
  ...
  var peg$f1 = function (die, modifiers) {
    die.modifiers = Object.assign(
      {},
      ...modifiers.map((item) => {
        return { [item.name]: item };
      })
  ...
  var peg$f12 = function (comparePoint) {
  ...
  var peg$f19 = function (l, expr, r) {
    return [l, ...expr, r];
  };
  var peg$f20 = function (func, expr) {
    return [`${func}(`, ...expr, ")"];
  };
  var peg$f21 = function (func, expr1, expr2) {
...
class DiceRoll {
   ...
   */
  toJSON() {
    const { averageTotal, maxTotal, minTotal, notation, output, rolls, total } =
      this;
  ...
  [rollMethodSymbol](engine) {
    ...
    const results = new ResultGroup(
      this[expressionsSymbol]
        .map((expression) => {
          if (
            expression instanceof StandardDice ||
            expression instanceof RollGroup
          ) {
            // roll the object and return the value
            return expression.roll();
          }
  ...
  [setRollsSymbol](rolls) {
    if (rolls instanceof ResultGroup) {
      ...
      this[rollsSymbol] = new ResultGroup(
        rolls.map((roll) => {
...
export {
  ...
  DiceRoll,
  DiceRoller,
  index$4 as Exceptions,
  index$1 as Modifiers,
  NumberGenerator$1 as NumberGenerator,
  Parser,
  index as Results,
  RollGroup,
  exportFormats,
